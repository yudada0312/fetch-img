...
//     });
// console.log(data);
console.log($('.o-other-total').eq(0).text().split(' ')[1].replace(/\D/g, ''));
console.log( $('.o-info-1').eq(1).find('dd').eq(1).text().split('\n')[0]);
console.log($('.oif-hl').eq(0).text());
    // 擷圖最終結果
    await taskHelper.takeScreenshots(formPage);
    return {
      type: 'CVS',
