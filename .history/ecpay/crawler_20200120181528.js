...
const waitForSelectorTimeout = 8000;
...
async function ATM(formPage) {
  try {
    ...
    return {
      type: 'ATM',
      //order_no: data[0],
      order_no: $3('dd').eq(0).text().trim(),
      //total_amount: data[10].replace(/\D/g, ''),
      total_amount:  $3('.o-other-total').eq(0).text().split(' ')[1].replace(/\D/g, ''),
     // expired_at: data[14],
      expired_at: $3('.o-info-1').eq(1).find('dd').eq(1).text().split('\n')[0],
      payment_no: '',
...
async function CVS(formPage) {
  try {
    ...
    return {
      type: 'CVS',
      order_no: $('dd').eq(0).text().trim(),
      total_amount:  $('.o-other-total').eq(0).text().split(' ')[1].replace(/\D/g, ''),
      expired_at: $('.o-info-1').eq(1).find('dd').eq(1).text().split('\n')[0],
      payment_no: $('.oif-hl').eq(0).text().trim(),
      atm_account: '',
      atm_bank: '',
      barcode1: '',
      barcode2: '',
